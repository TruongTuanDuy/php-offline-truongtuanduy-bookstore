...
class NewsModel extends Model
{
	...
	private $_columns = array('id', 'title', 'content', 'picture', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering', 'category_id');
	private $_userInfo;
	...
	{
		if ($option == null) {
			$query 	= "SELECT `id`, `name` FROM `" . TBL_CATEGORY_NEWS . "`";
	...
	{
		$query[]	= "SELECT `b`.`id`, `b`.`title`, `b`.`picture`, `b`.`status`, `b`.`ordering`, `b`.`created`, `b`.`created_by`, `b`.`modified`, `b`.`modified_by`, `c`.`name` AS `category_name`";
		$query[]	= "FROM `$this->table` AS `b` LEFT JOIN `" . TBL_CATEGORY_NEWS . "` AS `c` ON `b`.`category_id` = `c`.`id`";
		$query[]	= "WHERE `b`.`id` > 0";
	...
	public function changeStatus($arrParam, $option = null)
	{
		...
		if ($option['task'] == 'change-ajax-special') {
			...
			$result = array(
				'id'		=> $id,
				'special'	=> $special,
				'link'		=> URL::createLink('admin', 'news', 'ajaxSpecial', array('id' => $id, 'special' => $special))
			);
			return $result;
		}
		...
		if ($option == null) {
			if (!empty($arrParam['cid'])) {
				...
				foreach ($arrImage as $value) {
					$uploadObj->removeFile('news', $value);
					$uploadObj->removeFile('news', '98x150-' . $value);
				}
	...
	{
		if ($option == null) {
			$query[]	= "SELECT `id`, `content`, `picture`, `title`, `category_id`, `status`, `ordering`";
			$query[]	= "FROM `$this->table`";
	...
	public function saveItem($arrParam, $option = null)
	{
		...
		if ($option['task'] == 'add') {
			$arrParam['form']['picture']	= $uploadObj->uploadFile($arrParam['form']['picture'], 'news', 98, 150);
			$arrParam['form']['created']	= date('Y-m-d', time());
			$arrParam['form']['created_by']	= $this->_userInfo['username'];
			$arrParam['form']['content'] = mysqli_real_escape_string($arrParam['form']['content']);
			$arrParam['form']['title']		= mysqli_real_escape_string($arrParam['form']['title']);
		...
		if ($option['task'] == 'edit') {
			if ($arrParam['form']['picture']['name'] == null) {
				...
				$uploadObj->removeFile('news', '98x150-' .  $arrParam['form']['picture_hidden']);
