...
class CouponModel extends Model
{
	...
	private $_columns = array('id', 'code', 'value', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering');
	private $_userInfo;
	...
	public function listItem($arrParam, $option = null)
	{
		...
		$query[]	= "SELECT `id`, `code`, `value`, `status`, `ordering`, `created`, `created_by`, `modified`, `modified_by`";
		$query[]	= "FROM `$this->table`";
		$query[]	= "WHERE `id` > 0";
	...
	public function changeStatus($arrParam, $option = null)
	{
		if ($option['task'] == 'change-ajax-status') {
			...
			$result = array(
				'id'		=> $id,
				'status'	=> $status,
				'value'		=> URL::createLink('admin', 'coupon', 'ajaxStatus', array('id' => $id, 'status' => $status))
			);
			return $result;
		}
	...
	{
		if ($option == null) {
			$query[]	= "SELECT `id`, `code`, `value`, `status`, `ordering`";
			$query[]	= "FROM `$this->table`";
