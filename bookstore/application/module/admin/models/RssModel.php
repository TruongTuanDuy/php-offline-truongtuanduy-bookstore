...
class RSSModel extends Model
{
	...
	public function countItem($arrParam, $option = null)
	{
		...
		// FILTER : RSS ACP
		if (isset($arrParam['filter_link']) && $arrParam['filter_link'] != 'default') {
			$query[]	= "AND `link` = '" . $arrParam['filter_link'] . "'";
		}
	...
	public function changeStatus($arrParam, $option = null)
	{
		...
		if ($option['task'] == 'change-ajax-rss-acp') {
			$link 	= ($arrParam['link'] == 0) ? 1 : 0;
			$modified_by	= $this->_userInfo['username'];
			$modified		= date('Y-m-d', time());
			$id			= $arrParam['id'];
			$query		= "UPDATE `$this->table` SET `link` = $link, `modified` = '$modified', `modified_by` = '$modified_by'  WHERE `id` = '" . $id . "'";
			$this->query($query);

			$result = array(
				'id'		=> $id,
				'link'	=> $link,
				'link'		=> URL::createLink('admin', 'rss', 'ajaxACP', array('id' => $id, 'link' => $link))
			);
			return $result;
		}
